...
- a RDBMS is a relational database management system where data is stored in the form of rows and columns (records and fields).
- SQL is a query language used to interact with such system to query the data needed

1. Why do tables need a `primary key`?

- tables do not need a primary key, but it's considered VERY bad design if you don't have one. Primary keys will help you distinguish the different rows to be unique and minimize anomalies. If you need your table to be joinable on something or clustered, you need a primary key.
...
1. What do we need in order to have a _many to many_ relationship between two tables.
   -we need a third table (junction table that has keys from both tables)
